...
class ConfiguracionTab(ttk.Frame):
    ...
    def recargar_vista(self):
        ...
        ttk.Label(frame_principal, text="CONFIGURACIÓN DEL SISTEMA", font=("Segoe UI", 14, "bold")).pack(pady=(0, 15))
        
        # Negocio
        frame_negocio = ttk.LabelFrame(frame_principal, text="Información del Negocio", padding=10)
        frame_negocio.pack(fill="x", pady=5)
        ttk.Label(frame_negocio, text="Nombre:", font=("Segoe UI", 10)).pack(anchor="w", pady=2)
        self.entry_nombre_comercio = ttk.Entry(frame_negocio, width=35, font=("Segoe UI", 10))
        self.entry_nombre_comercio.insert(0, config.get("nombre_comercio", ""))
        self.entry_nombre_comercio.pack(fill="x", pady=2)
        
        # Apariencia
        frame_tema = ttk.LabelFrame(frame_principal, text="Apariencia", padding=10)
        frame_tema.pack(fill="x", pady=5)
        ttk.Label(frame_tema, text="Tema:", font=("Segoe UI", 10)).pack(anchor="w", pady=2)
        ...
        frame_umbral = ttk.Frame(frame_inventario)
        frame_umbral.pack(fill="x", pady=2)
        ttk.Label(frame_umbral, text="Umbral alertas:", font=("Segoe UI", 10)).pack(side="left")
        ...
        btn_aplicar = ttk.Button(frame_botones, text="💾 Aplicar", command=self._aplicar_configuracion, style="success.TButton", width=15)
